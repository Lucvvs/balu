...
def _bytes_to_rl_image(raw: bytes, max_w_pt: float, max_h_pt: float):
    """
    Convierte bytes de imagen (JPEG/PNG/WebP/…) a PNG RGB y devuelve RLImage
    ...
    # Limitar resolución raster para PDF ligero; algo mayor por imágenes ~30% más grandes en tarjeta
    max_px = 715
...
def _static_asset_to_png_dict(path: Path) -> dict | None:
    """Imagen estática (webp/png/jpg) → PNG RGB en buffer para ReportLab."""
    try:
        im = Image.open(path)
        im.load()
        if im.mode in ("RGBA", "LA"):
